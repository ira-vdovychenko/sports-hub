...
export const IAMenu = ({ item, itemType, isActive, onPress, onDelete, onEdit, name, isHidden, onPressChangeVisibility, onPressToMove, categories, subcategories,selectedCategory }) => {
  ...
  const handleMoveToSubcategory = (e, newSubcategoryId, newCategoryId) => {
    e.preventDefault();
    if (itemType === "team") {
      onPressToMove(newSubcategoryId, newCategoryId);
      setIsDropdownOpen(false);
      setIsHovering(false);
      setIsMoveToOpen(false);
      setActiveCategory("");
    ...
    <Styled.MenuItem onMouseOver={handleMouseOver} onMouseOut={handleMouseOut}>
                    ...
                    .map(
                      (cat) =>
                        !cat.isStatic && (
                          <Styled.DropDownMenuElement
                            key={cat.SportID}
                            onClick={(e) => {handleMoveToCategory(e, cat.SportID);}}
                            onMouseOver={() => {setHoveredCategory(cat);}}
                            onMouseOut={() => setHoveredCategory(null)}
                          >
                            <Styled.DropDownMenuElementText>
                              {cat.SportName}
                            </Styled.DropDownMenuElementText>
                            {hoveredCategory === cat && itemType === "team" && (
                              <Styled.SubcategoriesDropDownMenu>
                                {subcategories &&
                                  subcategories
                                    .filter((subcategory) =>subcategory.SportID === cat.SportID)
                                    .map((subcategory) => {
                                      return (
                                        <Styled.DropDownMenuElement
                                          key={subcategory.LeagueID}
                                          onClick={(e) => {handleMoveToSubcategory(e, subcategory.LeagueID, cat.SportID);}}
                                        >
                                          <Styled.DropDownMenuElementText>
      ...
      {isEditPopupVisible && (
        <Popup
          ...
          titleStyle={{ fontSize: 14, fontWeight: 700, paddingTop: '55px' }}
          description={"Are you sure?"}
          submitButton="Save"
          cancelButton="Cancel"
          showForm
